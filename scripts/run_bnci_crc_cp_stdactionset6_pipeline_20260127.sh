...
echo "[$(date)] === Run strict LOSO predictions (BNCI2014_001, 4-class; std action set 6) ==="

BASE_RUN="loso4_bnci2014_001_0train_paperfir_nc6_stdactionset6_v1"
...
for A in 0.35 0.40 0.50; do
  OUT="outputs/${DATE}/4class/offline_bnci2014_001_crc_cp_stdactionset6_alpha${A}_delta0.05_splits5_v1"
...
FIGDIR="docs/experiments/figures/${DATE}_bnci2014_001_crc_cp_stdactionset6_v1"
mkdir -p "${FIGDIR}"

for A in 0.35 0.40 0.50; do
  TAG="$(echo "${A}" | sed 's/\\./p/g')"
  CRC="outputs/${DATE}/4class/offline_bnci2014_001_crc_cp_stdactionset6_alpha${A}_delta0.05_splits5_v1"
  MERGED="outputs/${DATE}/4class/loso4_bnci2014_001_merged_plus_crc_cp_stdactionset6_alpha${A}_delta0.05_splits5_v1"
  ...
  python3 scripts/plot_candidate_diagnostics.py \
    ...
    --method "${METHOD_NAME}" \
    --out-dir "${FIGDIR}" \
    --prefix "bnci_crc_cp_stdactionset6_alpha${TAG}"
done

python3 scripts/plot_crc_risk_coverage.py \
  --run-dirs \
    "outputs/${DATE}/4class/offline_bnci2014_001_crc_cp_stdactionset6_alpha0.35_delta0.05_splits5_v1" \
    "outputs/${DATE}/4class/offline_bnci2014_001_crc_cp_stdactionset6_alpha0.40_delta0.05_splits5_v1" \
    "outputs/${DATE}/4class/offline_bnci2014_001_crc_cp_stdactionset6_alpha0.50_delta0.05_splits5_v1" \
  --method "${METHOD_NAME}" \
  --out-dir "${FIGDIR}" \
  --prefix bnci_crc_cp_stdactionset6_v1
