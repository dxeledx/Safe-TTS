...
echo "[$(date)] === Run strict LOSO predictions (Schirrmeister2017/HGD, 4-class) ==="

BASE_RUN="loso4_schirr2017_0train_rs50_nc4_ea_ea_fbcsp_v1"
...
for A in 0.25 0.30 0.40; do
  OUT="outputs/${DATE}/4class/offline_hgd_safettta_crc_cp_alpha${A}_delta0.05_splits5_v1"
  ...
  python3 scripts/offline_safe_tta_multi_select_crc_from_predictions.py \
    ...
    --candidate-methods ea-fbcsp-lda \
    --candidate-family-map ea-fbcsp-lda=fbcsp \
    --guard-threshold 0.5 \
...
FIGDIR="docs/experiments/figures/${DATE}_hgd_crc_cp_v1"
mkdir -p "${FIGDIR}"

for A in 0.25 0.30 0.40; do
  TAG="$(echo "${A}" | sed 's/\\./p/g')"
  CRC="outputs/${DATE}/4class/offline_hgd_safettta_crc_cp_alpha${A}_delta0.05_splits5_v1"
  MERGED="outputs/${DATE}/4class/loso4_hgd_merged_plus_crc_cp_alpha${A}_delta0.05_splits5_v1"
  ...
  python3 scripts/plot_candidate_diagnostics.py \
    ...
    --method "${METHOD_NAME}" \
    --out-dir "${FIGDIR}" \
    --prefix "hgd_crc_cp_alpha${TAG}"
done

python3 scripts/plot_crc_risk_coverage.py \
  --run-dirs \
    "outputs/${DATE}/4class/offline_hgd_safettta_crc_cp_alpha0.25_delta0.05_splits5_v1" \
    "outputs/${DATE}/4class/offline_hgd_safettta_crc_cp_alpha0.30_delta0.05_splits5_v1" \
    "outputs/${DATE}/4class/offline_hgd_safettta_crc_cp_alpha0.40_delta0.05_splits5_v1" \
  --method "${METHOD_NAME}" \
  --out-dir "${FIGDIR}" \
  --prefix hgd_crc_cp_v1
