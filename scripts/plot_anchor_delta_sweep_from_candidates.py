...
def _plot_sweep(df: pd.DataFrame, *, out_path: Path, title: str) -> None:
    ...
    x = df["anchor_guard_delta"].astype(float).to_numpy()
    ...
    axes[2].set_xlabel("anchor_guard_delta")
...
def main() -> None:
    ap = argparse.ArgumentParser(
        description="Offline anchor_guard_delta sweep from saved candidates.csv (no reruns)."
    )
    ap.add_argument(
        "--diagnostics-dir",
        type=Path,
    ...
    ap.add_argument(
        "--deltas",
        type=str,
        default="0,0.01,0.03,0.05,0.07,0.1",
        help="Comma-separated anchor_guard_delta sweep values.",
    )
    ap.add_argument("--guard-threshold", type=float, default=0.5)
    ap.add_argument("--probe-hard-worsen", type=float, default=-0.01)
    ap.add_argument("--global-min-pred-improve", type=float, default=0.0)
    ...
    ap.add_argument("--title", type=str, default="BNCI2014_001 4-class LOSO: anchor_guard_delta sweep")
    ap.add_argument("--prefix", type=str, default="bnci_anchor_delta_sweep")
    args = ap.parse_args()
    ...
    gates = SweepGates(
        ...
        global_min_pred_improve=float(args.global_min_pred_improve),
    ...
    for d in deltas:
        ...
        for s in subjects:
            out = _select_one_subject(cand_by_subject[s], anchor_guard_delta=float(d), gates=gates)
            per_subject_rows.append({"subject": int(s), "anchor_guard_delta": float(d), **out})
        ...
        sweep_rows.append(
            {
                ...
                "oracle_mean_accuracy": float(np.mean(np.asarray(oracle_acc, dtype=np.float64))),
                "oracle_mean_gap": float(np.mean(np.asarray(oracle_acc, dtype=np.float64) - sel_arr)),
    ...
    df_sweep = pd.DataFrame(sweep_rows).sort_values("anchor_guard_delta").reset_index(drop=True)
    df_subject = pd.DataFrame(per_subject_rows).sort_values(["anchor_guard_delta", "subject"]).reset_index(drop=True)
