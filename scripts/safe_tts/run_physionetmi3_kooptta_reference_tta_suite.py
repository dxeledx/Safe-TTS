...
DEFAULT_OUT_DIR = ROOT / "outputs" / "20260413" / "3class" / "ttime_suite_physionetmi3_kooptta_reference_seed0_v1"
DEFAULT_METHODS = (
...
def parse_args() -> tuple[argparse.Namespace, list[str]]:
    parser = argparse.ArgumentParser(
        description="Canonical wrapper for the KoopTTA-reference PhysioNetMI 3-class plain TTA suite."
    )
...
def main() -> int:
    ...
    target = ROOT / "scripts" / "ttime_suite" / "run_suite_loso_kooptta_reference.py"
    cmd = [
        sys.executable,
        str(target),
        "--data-dir",
