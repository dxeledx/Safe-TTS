...
DEFAULT_CANDIDATE_METHODS = "t3a,shot,sar,bn_adapt"
...
def _default_out_dir(date_prefix: str, risk_alpha: float) -> Path:
    ...
    return (
        ...
        / str(date_prefix)
        / "3class"
        / f"physio_safe_tts_stable_physionetmi3_alpha{alpha}_v1"
    )


def _default_method_name(risk_alpha: float) -> str:
    alpha = _alpha_tag(risk_alpha)
    return f"safe-tts-stable-evidential-physionetmi3-alpha{alpha}-v1"


def parse_args() -> tuple[argparse.Namespace, list[str]]:
    parser = argparse.ArgumentParser(
        description="Stable wrapper for Safe-TTS on PhysioNetMI 3-class."
    )
    parser.add_argument(
        "--preds",
        type=Path,
    ...
    parser.add_argument("--risk-alpha", type=float, default=0.35)
    ...
    parser.add_argument("--guard-threshold", type=float, default=0.55)
    ...
    parser.add_argument("--selector-views", type=str, default="stats,decision,relative,dynamic,stochastic")
    ...
    parser.add_argument("--threshold-score", type=str, default="pred_improve_x_guard")
    parser.add_argument("--candidate-choice-score", type=str, default="pred_improve_x_guard")
    parser.add_argument("--min-pred-grid", type=str, default="0,0.01,0.02,0.05,0.10,0.15,0.20")
...
def main() -> int:
    ...
    cmd = [
        ...
        str(args.candidate_choice_score),
        "--min-pred-grid",
