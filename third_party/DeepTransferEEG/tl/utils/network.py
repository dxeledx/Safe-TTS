...
def _infer_feature_dim(netF: nn.Module, *, chn: int, time_sample_num: int) -> int:
    # EEGNet_feature expects input shape: (batch, 1, Chans, Samples)
    with tr.no_grad():
        dummy = tr.zeros((1, 1, int(chn), int(time_sample_num)), dtype=tr.float32)
        out = netF(dummy)
    if out.ndim != 2 or out.shape[0] != 1:
        raise RuntimeError(f"Unexpected EEGNet_feature output shape: {tuple(out.shape)}")
...
def backbone_net(args, return_type='y'):
    netF = EEGNet_feature(n_classes=args.class_num,
                        ...
                        dropoutRate=0.25,
                        norm_rate=0.5)
    # Upstream code expects `args.feature_deep_dim` to be set manually for each dataset.
    # For new datasets / sampling rates (e.g. PhysioNetMI, Schirrmeister2017),
    # infer it automatically to avoid silent dimension mismatch.
    feature_dim = _infer_feature_dim(netF, chn=int(args.chn), time_sample_num=int(args.time_sample_num))
    if return_type == 'y':
        netC = FC(feature_dim, args.class_num)
    elif return_type == 'xy':
        netC = FC_xy(feature_dim, args.class_num)
    return netF, netC
