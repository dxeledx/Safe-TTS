...
def SAR(loader, model, args, balanced=True):
    """SAR (Sharpness-Aware and Reliable Test-Time Adaptation) — paper-aligned core steps.

    Reference: "Towards Stable Test-Time Adaptation in Dynamic Wild World" (ICLR 2023)

    Key components (Algorithm 1 in the paper appendix):
      - Reliable filtering: skip updates when entropy E(x;Θ) > E0.
      - Sharpness-aware update: SAM on entropy loss.
      - Recovery: if moving-average entropy em < e0, restore weights to initial values.

    For EEGNet we follow the paper's "optimize affine parameters in norm layers" guidance by
    updating only BatchNorm2d affine parameters (γ/β).
    """

    y_true = []
    y_pred = []

    # Trainable params: BN affine only (paper recommendation).
        ...
        if isinstance(m, nn.BatchNorm2d):
            for npp, p in m.named_parameters():
                if npp in ["weight", "bias"]:
                    ...
                    bn_affine_params.append(p)
    if not bn_affine_params:
        raise RuntimeError("SAR: no BatchNorm2d affine parameters found to optimize.")

    base_optimizer = torch.optim.Adam
    rho = float(getattr(args, "sar_rho", 0.05))
    optimizer = SAM(bn_affine_params, base_optimizer, lr=float(args.lr), rho=rho)

    # Reliable filtering threshold and recovery threshold (paper defaults).
    # Reference SAR repo uses `margin_e0 = 0.4 * log(K)` and recovery when EMA < 0.2.
    E0 = float(getattr(args, "sar_E0", 0.4 * math.log(float(args.class_num))))
    e0 = float(getattr(args, "sar_e0", 0.2))
    em = 0.0

    # Save initial BN affine params for recovery.
    init_params = [p.detach().clone() for p in bn_affine_params]

    def _recover():
        nonlocal optimizer, em
        for p, init in zip(bn_affine_params, init_params):
            p.data.copy_(init)
        optimizer = SAM(bn_affine_params, base_optimizer, lr=float(args.lr), rho=rho)
        em = 0.0
    ...
    for i in range(len(loader)):
        ...
        if (i + 1) >= args.test_batch and (i + 1) % args.stride == 0:
            ...
            for step in range(args.steps):
                ...
                # Reliable filtering (SAR): only use samples with entropy < E0.
                logits = model(inputs)[1]
                probs = nn.Softmax(dim=1)(logits / args.t)
                entropys = Entropy(probs)  # (B,)
                filter_ids_1 = torch.where(entropys < E0)[0]
                if filter_ids_1.numel() <= 0:
                    continue
                loss_first = entropys[filter_ids_1].mean(0)

                # SAM step 1 (at Θ)
                loss_first.backward()
                optimizer.first_step(zero_grad=True)

                # SAM step 2 (at Θ + ε_hat)
                logits_adv = model(inputs)[1]
                probs_adv = nn.Softmax(dim=1)(logits_adv / args.t)
                entropys2 = Entropy(probs_adv)[filter_ids_1]
                loss_second_value = entropys2.detach().mean(0)

                filter_ids_2 = torch.where(entropys2 < E0)[0]
                if filter_ids_2.numel() > 0:
                    loss_second = entropys2[filter_ids_2].mean(0)
                else:
                    # Still call backward/second_step to restore weights from SAM, but with zero grads.
                    loss_second = entropys2.mean(0) * 0.0

                loss_second.backward()
                optimizer.second_step(zero_grad=True)

                # Moving average entropy + recovery
                E_adv = float(loss_second_value.cpu().item())
                em = (0.9 * em + 0.1 * E_adv) if em != 0.0 else E_adv
                if em < e0:
                    _recover()
