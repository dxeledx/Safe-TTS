...
class Conv1dWithConstraint(nn.Conv1d):
    def __init__(self, *args, max_norm: float | None = None, **kwargs):
        self.max_norm = max_norm
        super().__init__(*args, **kwargs)

    def forward(self, x: Tensor) -> Tensor:
        if self.max_norm is not None:
            self.weight.data = torch.renorm(self.weight.data, p=2, dim=0, maxnorm=float(self.max_norm))
        return super().forward(x)


class ChannelGroupAttention(nn.Module):
    """Channel Group Attention (grouped squeeze-excitation on channel groups)."""
    ...
    def __init__(self, in_channels: int, num_groups: int, reduction: int = 4):
        ...
        self.group_size = int(in_channels // num_groups)

        self.pool = nn.AdaptiveAvgPool2d(1)
        self.att_fc1 = nn.Conv2d(
            in_channels, in_channels // reduction, kernel_size=1, groups=num_groups, bias=False
        )
    ...
    def forward(self, x: Tensor) -> Tensor:
        ...
        if c != self.in_channels:
            raise ValueError(
                f"Input channel dimension ({c}) does not match configured in_channels ({self.in_channels})"
            )

        pooled = self.pool(x)
        group_att = self.att_fc1(pooled)
        att_weights = self.att_fc2(self.relu(group_att))
        att_weights = self.sigmoid(att_weights)  # [B, G, 1, 1]

        x_reshaped = x.view(b, self.num_groups, self.group_size, h, w)
        att_weights = att_weights.view(b, self.num_groups, 1, 1, 1)
        out = (x_reshaped * att_weights).view(b, c, h, w)
        return out
...
class TCFormerNet(nn.Module):
    """Skorch/Braindecode-friendly wrapper around `TCFormerModule` (outputs logits)."""
    ...
    def __init__(
        ...
        *,
        ...
        super().__init__()
        _ = n_times  # kept for API compatibility with other Braindecode modules
        self.model = TCFormerModule(
            n_channels=int(n_chans),
            n_classes=int(n_outputs),
            F1=int(F1),
            temp_kernel_lengths=tuple(int(k) for k in temp_kernel_lengths),
            pool_length_1=int(pool_length_1),
            pool_length_2=int(pool_length_2),
            D=int(D),
            dropout_conv=float(dropout_conv),
            d_group=int(d_group),
            tcn_depth=int(tcn_depth),
            kernel_length_tcn=int(kernel_length_tcn),
            dropout_tcn=float(dropout_tcn),
            use_group_attn=bool(use_group_attn),
            q_heads=int(q_heads),
